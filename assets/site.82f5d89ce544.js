...
// Footer navigation stays visible without JS and collapses only on phones.
document.querySelectorAll('.footer-nav-group').forEach((group, index) => {
  const label = group.querySelector('.footer-label');
  const links = group.querySelector('.footer-links');
  if (!label || !links) return;
  const toggle = document.createElement('button');
  toggle.type = 'button';
  toggle.className = 'footer-nav-toggle footer-label';
  toggle.textContent = label.textContent;
  links.id = `footer-navigation-${index}`;
  toggle.setAttribute('aria-controls', links.id);
  label.after(toggle);
  const mobile = matchMedia('(max-width: 640px)');
  const sync = () => {
    toggle.hidden = !mobile.matches;
    label.hidden = mobile.matches;
...
// Wheel inertia only: touch, keyboard and nested scrollers remain native.
(() => {
  const pointer = matchMedia('(any-pointer: fine)');
  const speed = 0.5;
  const dampingMs = 145;
  let frame = 0, target = scrollY, position = scrollY, lastWritten = scrollY;
  let previousTime = 0, direction = 0;
  const stop = () => {
  ...
  const tick = (time) => {
    if (Math.abs(scrollY - lastWritten) > 2 || reducedMotion.matches || blocked()) {
      stop();
      return;
    }
    ...
    position += (target - position) * (1 - Math.exp(-elapsed / dampingMs));
    const finished = Math.abs(target - position) < 0.5;
    if (finished) position = target;
    window.scrollTo({top: position, behavior: 'instant'});
    lastWritten = scrollY;
    frame = finished ? 0 : requestAnimationFrame(tick);
  };
  window.addEventListener('wheel', (event) => {
    if (event.defaultPrevented || !event.cancelable || reducedMotion.matches || !pointer.matches
      || event.ctrlKey || event.metaKey || event.shiftKey || !event.deltaY
      || Math.abs(event.deltaX) > Math.abs(event.deltaY) || blocked()) return;
    for (const element of event.composedPath()) {
      if (!(element instanceof Element) || element === document.body || element === document.documentElement) continue;
      if (element.matches('input, textarea, select, [contenteditable]:not([contenteditable="false"]), [role="dialog"]')) return;
      if (element.scrollHeight > element.clientHeight + 1
        && /auto|scroll|overlay/.test(getComputedStyle(element).overflowY)) return;
    }
    const unit = event.deltaMode === 1 ? 16 : event.deltaMode === 2 ? innerHeight : 1;
    const delta = Math.max(-500, Math.min(500, event.deltaY * unit)) * speed;
    const nextDirection = Math.sign(delta);
    if (!frame || (direction && direction !== nextDirection)) target = position = lastWritten = scrollY;
  ...
  for (const type of ['pointerdown','touchstart','hashchange','resize','blur','pagehide']) {
